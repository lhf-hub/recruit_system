...
const getResumes = async () => {
  ...
  try {
    ...
    res.data.data.forEach((item: ResumeInfo) => {
      // if (item.writtenTime == null) item.writtenTime = new Date(Date.now()).toLocaleString().split(' ')[0]
      // if (item.interviewTime == null) item.interviewTime = new Date(Date.now()).toLocaleString().split(' ')[0]
...
const doShowWrittenModal = ref(false)
const openWrittenModal = (resume: ResumeInfo) => {
  editedItem.value = {
  ...
  doShowWrittenModal.value = true
}

const writtenResume = async () => {
  console.log(editedItem.value.writtenTime.toLocaleString().split(' ')[0].replace(/\//g, "-") + ' ' + editedItem.value.writtenDetailTime.toLocaleString().split(' ')[1])
  if (!(editedItem.value?.writtenTime && editedItem.value?.writtenDetailTime)) {
    bus.emit('alert', { type: 'warning', content: '请完整填写笔试时间' })
    return
  }
  if (

    new Date(
      `${editedItem.value?.writtenTime.toLocaleString().split(' ')[0]} ${editedItem.value?.writtenDetailTime
        .toLocaleString()
        .split(' ')[1]}`,
    ) < new Date(Date.now())
  ) {
    bus.emit('alert', { type: 'warning', content: '笔试时间不能早于当前时间' })
    return
  }
  try {
    const response = await application.updateIn({
      applicationId: editedItem.value?.applicationId,
      writtenTime: `${editedItem.value?.writtenTime.toLocaleString().split(' ')[0].replace(/\//g, "-")} ${editedItem.value?.writtenDetailTime
        .toLocaleString()
        .split(' ')[1]
        .replace(/\//g, "-")}`,
    })
    if (response.data.code !== 200) {
      ...
      bus.emit('alert', { type: 'success', content: '约笔试成功' })
      doShowWrittenModal.value = false
    }
  } catch (e) {
    bus.emit('alert', { type: 'error', content: '网络错误' })
  }
...
const interviewResume = async () => {
  ...
  if (
    new Date(
      `${editedItem.value?.interviewTime.toLocaleString().split(' ')[0]} ${editedItem.value?.interviewDetailTime
        .toLocaleString()
        .split(' ')[1]}`,
    ) < new Date(Date.now())
  ) {
...
<template>
  <VaCard>
    <VaCardTitle text-color="primary" class="page-title">简历管理</VaCardTitle>
    <VaCardContent>
      <ResumeCards
        :resumes="resumes"
        :loading="isLoading"
    ...
    <VaModal v-model="doShowWrittenModal" hide-default-actions size="small" close-button stateful>
      <span style="font-weight: bold; font-size: x-large">请选择笔试时间</span>
      <div style="display: flex; justify-content: center; gap: 20px; padding: 20px">
        <VaDateInput v-model="editedItem.writtenTime" />
        <VaTimeInput v-model="editedItem.writtenDetailTime" />
      </div>
      <template #footer>
        <div style="display: flex; align-items: end; gap: 8px">
          <VaButton preset="secondary" text-color="#999" @click="doShowWrittenModal = false">取消</VaButton>
          <VaButton @click="writtenResume">确定</VaButton>
        </div>
      </template>
    </VaModal>
